...
from cellscape.spatial.annotation import update_obs_columns_from_dataframe
...
def test_update_obs_columns_from_dataframe_updates_matched_rows_only() -> None:
    ...
    update_obs_columns_from_dataframe(
        ...
        replace_columns=["new_x", "new_y"],
        replaced_columns=["x", "y"],
    )
...
def test_update_obs_columns_from_dataframe_creates_missing_target_column() -> None:
    """目标列不存在时应创建 NA 列并仅写入匹配行的新值。"""
    adata = _MiniAnnData(pd.DataFrame({"cell_id": ["c1", "c2"]}))
    df = pd.DataFrame({"cell_id": ["c2"], "new_region": ["tumor"]})

    with pytest.warns(UserWarning, match="缺少被替换列"):
        update_obs_columns_from_dataframe(
            adata,
            df,
            index_columns="cell_id",
            replace_columns="new_region",
            replaced_columns="region",
        )
...
def test_update_obs_columns_from_dataframe_rejects_duplicate_df_keys() -> None:
    ...
    with pytest.raises(ValueError, match="df.*索引不唯一"):
        update_obs_columns_from_dataframe(
            ...
            df,
            index_columns="cell_id",
            replace_columns="region",
            replaced_columns="region",
        )


def test_update_obs_columns_from_dataframe_rejects_empty_match() -> None:
...
def test_update_obs_columns_from_dataframe_warns_duplicate_obs_keys_when_allowed() -> None:
...
def test_update_obs_columns_from_dataframe_can_return_copy() -> None:
    """非原位模式应返回更新后的副本, 并保持原始 adata 不变。"""
    adata = _MiniAnnData(pd.DataFrame({"cell_id": ["c1"], "region": ["old"]}))
    df = pd.DataFrame({"cell_id": ["c1"], "region": ["new"]})

    result = update_obs_columns_from_dataframe(
        adata,
        df,
        index_columns="cell_id",
        replace_columns="region",
